...
export default function Bags() {
        ...
        <section key={b.id}>
            <div className='flex justify-center items-center border-2 text-[#2e4053] group border-[#f7f7f7] hover:bg-[#2e4053] duration-300 hover:text-white hover:shadow-sm flex-col'>
                <Link href={`/product/${b.name.split(' ').join('-')}`}>
                    <h2 className='ml-1 sm:ml-0 sm:text-3xl text-left my-4 font-bold'>{b.name}</h2>
                    <p className='ml-1 sm:ml-0 sm:text-xl text-left font-semibold'><span>&#8358;</span><span>{b.amount.toLocaleString()}</span></p>
                    <section>
  ...
  return (
    <div>
        <h1 className='text-4xl uppercase font-bold text-center font-serif text-[#2e4053]'>Bags</h1>


        <div className='grid sm:grid-cols-3 grid-cols-2 gap-2 sm:gap-4 mx-4 md:mx-8 my-5'>
            {bagLog}
        </div>
